...
# coarse scale states
num_coarse_states <-  1800 # 1 hour #300 # 10 mins
...
### one array task = one simulation run, indexed by SLURM_ARRAY_TASK_ID
i <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID"))
if (is.na(i)) stop("SLURM_ARRAY_TASK_ID is not set - are you running under sbatch --array?")

cat("Running simulation iteration", i, "\n")

res <- run_one_iter(i, df)

dir.create("sim_results", showWarnings = FALSE)
saveRDS(res, sprintf("sim_results/run_%04d.rds", i))

cat("Saved sim_results/run_", sprintf("%04d", i), ".rds\n", sep = "")
