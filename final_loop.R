...
coarse_gamma <- matrix(c(0.602, 0.119, 0.279, 0.364, 0.418, 0.218, 0.383, 0.081, 0.538), nrow = 3, byrow = TRUE)
# solve for stationary distribution 
delta <- solve(t(diag(3) - coarse_gamma + 1), rep(1, 3))

# coarse scale states
num_coarse_states <- 1800 # 1 hour #300 # 10 mins #900 # 30 mins 
...
get_allprobs <- function(theta.star, x, N, scenario) {
  ...
  if (!scenario %in% c('base_acc')) {
    ...
    mu_step <- rev(cumsum(rev(deltas)))
    
    sig_step <- exp(theta.star[base + N + 1:N])
    mu_angle <- theta.star[base + 2*N + 1:N]        # not exponentiated, can be negative
    kappa <- exp(theta.star[base + 3*N + 1:N])   # must be positive
    kappa <- pmax(pmin(kappa, 1e4), 1e-6) # stops extreme k values
  }
    
  
  if (scenario %in% c('ideal', 'coarse')) {
    mu_acc <- exp(theta.star[base + 4*N + 1:N])
    sd_acc <- exp(theta.star[base + 5*N + 1:N])
  } else if (scenario %in% c('base_acc')) {
    deltas <- exp(theta.star[base + 1:N])
    mu_acc <- rev(cumsum(rev(deltas)))
    sd_acc <- exp(theta.star[base + N + 1:N])
  } else if (scenario == 'mean_summary') {
    mu_acc <- exp(theta.star[base + 4*N + 1:N])
    sd_acc <- exp(theta.star[base + 5*N + 1:N])
    sd_acc_mean <- sd_acc
  } else if (scenario == 'var_summary') {
    mu_acc_var <- exp(theta.star[base + 4*N + 1:N])
    sd_acc_var <- exp(theta.star[base + 5*N + 1:N])
  } else if (scenario == 'med_summary') {
    mu_acc <- exp(theta.star[base + 4*N + 1:N])
    sd_acc <- exp(theta.star[base + 5*N + 1:N])
    sd_acc_med <- sd_acc #* sqrt(pi/2) 
  } else if (scenario == 'stats_summary') {
    mu_acc <- theta.star[base + 4*N + 1:N] # unrestricted since normalized
    sd_acc <- exp(theta.star[base + 5*N + 1:N])
    sd_acc_mean <- sd_acc
    
    mu_acc_var <- theta.star[base + 6*N + 1:N] # unrestricted since normalized
    sd_acc_var <- exp(theta.star[base + 7*N + 1:N])
    
    mu_acc_med <- theta.star[base + 8*N + 1:N]
    sd_acc_med <- exp(theta.star[base + 9*N + 1:N])
  } else if (scenario == 'coarse_abs') {
    mu_acc <- exp(theta.star[base + 4*N + 1:N]) # exp because it must be positive
    sd_acc <- exp(theta.star[base + 5*N + 1:N])
    ...
    mu_comp <- mu_comp / rowSums(mu_comp)
    
    # concentration parameter per state (controls spread around mu_j)
    phi <- exp(theta.star[base + 4*N + (K-1)*N + 1:N])
  } else if  (scenario %in% c('quantile_summary')){
    K <- k
    quant_cols <- 3:(3+K-1)
    
    # model each quantile with a gamma distribution
    mu_q <- matrix(exp(theta.star[base + 4*N + 1:(K*N)]), nrow = N, ncol = K, byrow = TRUE)
    sd_q <- matrix(exp(theta.star[base + 4*N + K*N + 1:(K*N)]), nrow = N, ncol = K, byrow = TRUE)
...
mllk <- function(theta.star, x, N, scenario='base') {
  p <- get_allprobs(theta.star, x, N, scenario=scenario)
  n_obs <- nrow(x)
...
generate_cluster_start <- function(x, N, scenario, iter) {
  print("using cluster")
...
generate_random_start <- function(x, N, scenario) {
...
run_multiple_starts <- function(x, N, n_starts = 20, scenario, use_clustering = TRUE) {
  # store all results
  ...
  for (i in 1:n_starts) {
    ...
    if (i == 1 && use_clustering) {   # first iteration: use clustering
      theta_start <- generate_cluster_start(x, N, scenario, iter = i)
    } else {                          # subsequent iterations: random perturbations
      theta_start <- generate_random_start(x, N, scenario) 
    }
    
    # run optimization
    mod <- tryCatch({
      nlm(mllk, theta_start, x = x, N = N, scenario = scenario, iterlim = 10000, stepmax = 10, print.level = 0) #### try a different optimizer -- optim 
    }, error = function(e) {
...
viterbi <- function(theta.star, x, N, scenario) {
  p <- get_allprobs(theta.star, x, N, scenario)
  n_obs <- nrow(x)
...
#######################################################################################################################################
################################################## For-loop for multiple simulations ################################################## 
#######################################################################################################################################
# create data frame to store results
n_sims <- 20

output <- data.frame(
  acc_ideal = numeric(n_sims),
  acc_base_gps = numeric(n_sims),
  acc_base_acc = numeric(n_sims),
  acc_coarse = numeric(n_sims),
  acc_summary_mean = numeric(n_sims),
  acc_summary_var = numeric(n_sims),
  acc_summary_med = numeric(n_sims),
  acc_summary_stats = numeric(n_sims),
  acc_summary_density = numeric(n_sims),
  acc_summary_quantile = numeric(n_sims)
)  

cm_results <- vector("list", n_sims)
...
for (i in 1:n_sims) {
  ...
  #write.csv(full_data, file = "fulldata2")
  
  # sensor data -- remove gps except for at markers
  sensored_data <- full_data
  sensored_gps_data <- sensored_data[which(sensored_data$time %% fine_scale_points == 0), c('time', 'x', 'y')] # 10 minutes is fine_scale_points seconds
  
  # compute step length and turning angle 
  sensored_gps_data <- prepData(sensored_gps_data, type = 'UTM', coordNames = c("x", "y"))
  
  #plot(sensored_gps_data$x, sensored_gps_data$y, type = "l")
  
  # rejoin data
  sensored_data <- merge(sensored_gps_data, sensored_data[,c('states', 'time', 'acc')], by = c('time'), all.y = TRUE) # right outer join
  
  # GPS data only at the coarse-scale
  baseline_data <- sensored_data
  ...
  table(decoded = decoded_states, true = full_data$states)
  acc_ideal <- sum(decoded_states == full_data$states) / length(full_data$states)
  acc_ideal
  ...
  # # Step parameters
  # deltas <- exp(mod$estimate[base + 1:N]) 
  # mu_step <- rev(cumsum(rev(deltas)))
  # sig_step <- exp(mod$estimate[base + N + 1:N])
  # # Angle parameters
  # mu_angle <- mod$estimate[base + 2*N + 1:N]
  # kappa <- exp(mod$estimate[base + 3*N + 1:N])
  # # Acceleration parameters (ideal scenario)
  # mu_acc <- exp(mod$estimate[base + 4*N + 1:N])
  # sd_acc <- exp(mod$estimate[base + 5*N + 1:N])
  # 
  # cat("ESTIMATED PARAMETERS:\n")
  # cat("Step means:", round(mu_step, 3), "\n")
  # cat("Acc means:", round(mu_acc, 3), "\n\n")
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = baseline_data$states) 
  acc_base_gps <- sum(decoded_states == baseline_data$states) / length(baseline_data$states)
  acc_base_gps
  
  a <- accuracy(decoded_states = decoded_states, true_states =  baseline_data$states, N = N)
  acc_base_gps_cm <- a$best_cm
  print(acc_base_gps_cm)
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = full_data$states) 
  acc_base_acc <- sum(decoded_states == full_data$states) / length(full_data$states)
  acc_base_acc
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = coarse_data$states)
  acc_coarse <- sum(decoded_states == coarse_data$states) / length(coarse_data$states)
  acc_coarse
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = agg_mean$states)
  acc_summary_mean <- sum(decoded_states == agg_mean$states) / length(agg_mean$states)
  acc_summary_mean
  
  a <- accuracy(decoded_states = decoded_states, true_states =  agg_mean$states, N = N)
  acc_summary_mean_cm <- a$best_cm
  print(acc_summary_mean_cm)
  acc_summary_mean <- a$best_acc
  print(acc_summary_mean)
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = agg_var$states)
  acc_summary_var <- sum(decoded_states == agg_var$states) / length(agg_var$states)
  acc_summary_var
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = agg_med$states)
  acc_summary_med <- sum(decoded_states == agg_med$states) / length(agg_med$states)
  acc_summary_med
  ...
  # compare decoded states to true states
  table(decoded = decoded_states, true = agg_stats$states)
  acc_summary_stats <- sum(decoded_states == agg_stats$states) / length(agg_stats$states)
  acc_summary_stats
  
  a <- accuracy(decoded_states = decoded_states, true_states =  agg_stats$states, N = N)
  acc_summary_stats_cm <- a$best_cm
  print(acc_summary_stats_cm)
  acc_summary_stats <- a$best_acc
  print(acc_summary_stats)
  
  # # looking into performance
  # print(tapply(agg_med$acc_med, agg_med$states, summary))
  # by(agg_stats[, c("acc_mean", "acc_var")], agg_stats$states, 
  #    function(d) cor(d$acc_mean,d$acc_var, use = "complete.obs"))
  # print(tapply(agg_mean$acc_mean, agg_mean$states, summary))
  # 
  # boxplot(acc_mean ~ states,
  #         data = agg_mean,
  #         col = rainbow(length(unique(agg_mean$states))),
  #         main = "acc_mean by State",
  #         xlab = "State",
  #         ylab = "acc_mean")
  # 
  # boxplot(acc_mean ~ states,
  ...
  #         ylab = "acc_mean")
  # 
  # boxplot(acc_var ~ states,
  #         data = agg_var,
  #         col = rainbow(length(unique(agg_var$states))),
  #         main = "acc_var by State",
  #         xlab = "State",
  #         ylab = "acc_var")
  # 
  ...
  agg_density <- merge(sensored_data, density_wide, by = c("time"), all.x = TRUE)
  ...
  decoded_states <- viterbi(mod$estimate, x = cbind(agg_density$step, agg_density$angle, as.matrix(agg_density[, paste0("dens_", 1:k)])), N = N, scenario = 'density_summary')
  
  table(decoded = decoded_states, true = agg_density$states)
  acc_summary_density <- sum(decoded_states == agg_density$states) / length(agg_density$states)
  acc_summary_density
  
  a <- accuracy(decoded_states = decoded_states, true_states =  agg_density$states, N = N)
  acc_summary_density_cm <- a$best_cm
  print(acc_summary_density_cm)
  acc_summary_density <- a$best_acc
  print(acc_summary_density)
  ...
  agg_quantile <- merge(sensored_data, quantile_wide, by = c("time"), all.x = TRUE)
  ...
  decoded_states <- viterbi(mod$estimate, x = cbind(agg_quantile$step, agg_quantile$angle, as.matrix(agg_quantile[, paste0("dens_", 1:k)])), N = N, scenario = 'quantile_summary')
  
  table(decoded = decoded_states, true = agg_quantile$states)
  acc_summary_quantile <- sum(decoded_states == agg_quantile$states) / length(agg_quantile$states)
  acc_summary_quantile
  
  a <- accuracy(decoded_states = decoded_states, true_states =  agg_quantile$states, N = N)
  acc_summary_quantile_cm <- a$best_cm
  print(acc_summary_quantile_cm)
  acc_summary_quantile <- a$best_acc
  print(acc_summary_quantile)
  
  
  ################################################## Accuracy Summary ################################################## 
  cat("acc_ideal:", acc_ideal, "\n",
...
# output1 <- output
# cm_results1 <- cm_results
# acc1 <- colMeans(output1)
# write.csv(output1, "output1.csv")
# saveRDS(cm_results1, "cm_results1.rds")

# output2 <- output
# cm_results2 <- cm_results
# acc2 <- colMeans(output2)
# write.csv(output2, "output_hard.csv")
# saveRDS(cm_results2, "cm_results_hard.rds")

# output3 <- output
# cm_results3 <- cm_results
# acc3 <- colMeans(output3)
# write.csv(output3, "output_easy.csv")
# saveRDS(cm_results3, "cm_results_easy.rds")

# output4 <- output
# cm_results4 <- cm_results
# acc4 <- colMeans(output4)
# write.csv(output4, "output_short.csv")
# saveRDS(cm_results4, "cm_results_short.rds")

output5 <- output
cm_results5 <- cm_results
acc5 <- colMeans(output5)
write.csv(output5, "output_longer.csv")
saveRDS(cm_results5, "cm_results_longer.rds")


# second_output <- output
# second_cm_results <- cm_results
# second_acc <- colMeans(second_output)
# 
# write.csv(first_output, "first_output.csv")
# write.csv(second_output, "second_output.csv")
# 
# saveRDS(second_cm_results, "second_cm.rds")
