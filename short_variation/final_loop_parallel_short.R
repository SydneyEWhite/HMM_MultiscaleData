...
# coarse scale states
num_coarse_states <-  300 # 10 mins #900 # 30 mins # 1800 # 1 hour #300 # 10 mins
...
unpack_pars <- function(theta.star, N, scenario, k = NULL) {
  ...
  if (scenario %in% c('ideal', 'coarse')) {
    ...
    # to keep the optimization unconstrained, we are actually estimating the log ratio values of mu
         # each of the 6 raw_mu are the log ratio of the mu_comp_i to mu_comp_7, so we need to extract the actual mu
         # raw_mu_i = log(mu_comp_i / mu_comp_7)
         # exp(raw_mu_i) = mu_comp_i / mu_comp_7
    # since log(mu_comp_7/mu_comp_7) = log(1) = 0, we create a column of 0s (because the 7th row isn't in the original since it would be overparameterized)
...
run_one_iter <- function(i, df) {
  ...
  # per interation containers (replaces the param_results[[i]] list)
  param_i <- list()
  ...
  sensored_gps_data <- sensored_data[which(sensored_data$time %% fine_scale_points == 0), c('time', 'x', 'y')] 
  ...
  sensored_gps_data$step  <- c(NA, head(sensored_gps_data$step,  -1)) 
  sensored_gps_data$angle <- c(NA, head(sensored_gps_data$angle, -1))
...
acc <- colMeans(output)
write.csv(output, "output_fix_short.csv")
saveRDS(cm_results, "cm_results_fix_short.rds")
saveRDS(param_results, "param_results_fix_short.rds")
